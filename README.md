...
For openers that reverse when pressed while closing, `closing_stop_delay` sends the required second press after the reversal. It measures from the start of the first relay pulse to the start of the second. The effective delay is never shorter than `button_press_interval` plus a 200 ms safety margin. Partial closing targets compensate for that brief reverse travel so the door stops near the requested position.
